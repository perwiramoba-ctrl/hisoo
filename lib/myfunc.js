...
exports.tanggal = (numer) => {
	myMonths = ["Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"];
				...
				return`${thisDay}, ${day} - ${myMonths[bulan]} - ${year} - ${time}`
}
...
exports.generateProfilePicture = async (buffer) => {
  ...
  return {
    img: await jimp.getBuffer('image/jpeg'),
    preview: await jimp.getBuffer('image/jpeg')
  }
}
...
exports.imga = async (buffer) => {
  if (typeof buffer === 'string') buffer = fs.readFileSync(buffer)
  const jimp = await Jimp.fromBuffer(buffer)
  const min = jimp.width
  jimp.crop({ x: 0, y: 0, w: min, h: min })
  jimp.scaleToFit({ w: 720, h: 720 })
  return {
    img: await jimp.getBuffer('image/png'),
    preview: await jimp.getBuffer('image/png')
  }
}
...
exports.smsg = (conn, m/*, store*/) => {
	...
	if (m.message) {
		...
		m.msg = (m.mtype == 'viewOnceMessageV2' ? m.message[m.mtype].message[getTypeMessage(m.message[m.mtype].message)] : m.message[m.mtype]) ? (m.mtype == 'productMessage' ? m.message[m.mtype].message[getTypeMessage(m.message[m.mtype].message)] : m.message[m.mtype]) : m.message[m.mtype]
